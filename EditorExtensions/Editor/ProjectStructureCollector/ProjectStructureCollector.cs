...
namespace SkyClerik.Develop
{
    ...
    public class ProjectStructureCollectorHubPlugin : IEditorStartupPlugin
    {
        public void OnEditorStartup()
        {
#if UNITY_EDITOR
            if (!TryGetHubRegistryType(out var registryType, out var registerMethod))
                return; // хаб/реестр отсутствует — тихо выходим

            RegNode1(registerMethod);
            RegNode2(registerMethod);

            void RegNode1(MethodInfo registerMethod)
            {
                var icon = EditorStartupRunner.FindIconByName("sc_editor_structure");
                var node = new EditorHubNodeData
                {
                    Id = "ShowDirectoryStructureWindow",
                    ButtonTitle = "ShowDirectoryStructureWindow",
                    Description = "Собрать в список только файлы",
                    Group = "Window",
                    Icon = icon,
                    Action = () => ProjectStructureCollector.ShowDirectoryStructureWindow()
                };
                registerMethod.Invoke(null, new object[] { node });  // Invoke принимает object[] из-за сигнатуры API, но внутри только один аргумент — node
            }

            void RegNode2(MethodInfo registerMethod)
            {
                var icon = EditorStartupRunner.FindIconByName("sc_editor_structure");
                var node = new EditorHubNodeData
                {
                    Id = "ShowFullProjectStructureWindow",
                    ButtonTitle = "ShowFullProjectStructureWindow",
                    Description = "Собрать в список файлы и классы. (НЕ РЕКОМЕНДУЕТСЯ) Может сильно нагружать систему",
                    Group = "Window",
                    Icon = icon,
                    IconColor = Color.yellow,
                    Action = () => ProjectStructureCollector.ShowFullProjectStructureWindow()
                };
                registerMethod.Invoke(null, new object[] { node });  // Invoke принимает object[] из-за сигнатуры API, но внутри только один аргумент — node
            }

            bool TryGetHubRegistryType(out Type registryType, out MethodInfo registerMethod)
            {
                registryType = null;
                registerMethod = null;

                // Ищем по namespace без жёсткого assembly
                string typeName = "SkyClerik.Develop.EditorHubRegistry";
                foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                {
                    registryType = assembly.GetType(typeName);
                    if (registryType != null)
                    {
                        registerMethod = registryType.GetMethod("Register", BindingFlags.Public | BindingFlags.Static);
